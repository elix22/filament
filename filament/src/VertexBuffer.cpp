...
void FVertexBuffer::setBufferObjectAt(FEngine& engine, uint8_t bufferIndex,
        FBufferObject const * bufferObject, uint32_t byteOffset) {
    ...
    if (bufferIndex < mBufferCount) {
        ...
        engine.getDriverApi().setVertexBufferObject(mHandle,
                bufferIndex, hwBufferObject, byteOffset);
    } else {
        ASSERT_PRECONDITION(bufferIndex < mBufferCount, "bufferIndex must be < bufferCount");
    }
}
...
void VertexBuffer::setBufferObjectAt(Engine& engine, uint8_t bufferIndex,
        BufferObject const* bufferObject, uint32_t byteOffset) {
    upcast(this)->setBufferObjectAt(upcast(engine), bufferIndex, upcast(bufferObject), byteOffset);
}
